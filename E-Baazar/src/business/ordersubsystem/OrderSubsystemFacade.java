...
public class OrderSubsystemFacade implements OrderSubsystem {
	...
	public List<Order> getOrderHistory() throws BackendException {
		...
		try {
			...
			System.out.println("OrderSS   getOrderHistory   orderIds: " + orderIds);
			for (int i = 0; i < orderIds.size(); i++) {

				OrderImpl orderData = dbClass.getOrderData(orderIds.get(i));
				orderList.add(orderData);
			}
			System.out.println("OrderSS   getOrderHistory   orderList: " + orderList);
	...
	public void submitOrder(ShoppingCart cart) throws BackendException {
		...
		for (CartItem cartItem : cart.getCartItems()) {
			// calculate unit price by dividing total price by qty, since no
			// helper method exists in cartItem for that purpose.
	...
	public static Order createOrder(Integer orderId, String orderDate, String totalPrice) {
		// implement
		LOG.warning(
				"Method  createOrder(Integer orderId, String orderDate, String totalPrice) still needs to be implemented");
		return null;
	}
