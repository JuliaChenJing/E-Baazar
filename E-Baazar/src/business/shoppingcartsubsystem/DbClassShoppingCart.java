...
class DbClassShoppingCart implements DbClass, DbClassCartItemForTest {
	...
	enum Type {
		GET_ID, GET_SAVED_ITEMS, GET_TOP_LEVEL_SAVED_CART, SAVE_CART, SAVE_CART_ITEM, DELETE_CART, DELETE_ALL_CART_ITEMS
	};
	...
	private Object[] getIdParams, saveCartParams, getTopLevelSavedCartParams, getSavedItemsParams, saveCartItemParams,
			deleteCartParams, deleteAllCartItemsParams;
	private int[] getIdTypes, saveCartTypes, getTopLevelSavedCartTypes, getSavedItemsTypes, saveCartItemTypes,
			deleteCartTypes, deleteAllCartItemsTypes;
	...
	private Integer getShoppingCartId(CustomerProfile custProfile) throws DatabaseException {
		...
		dataAccessSS.read(); // value from read is stored in cartId instance
								// variable
		return cartId;
	}
	...
	private int saveCartTopLevel(CustomerProfile custProfile) throws DatabaseException {
		...
		saveCartParams = new Object[] { custProfile.getCustId(), cart.getShippingAddress().getStreet(),
				...
				cart.getBillingAddress().getZip(), cart.getPaymentInfo().getNameOnCard(),
				cart.getPaymentInfo().getExpirationDate(), cart.getPaymentInfo().getCardType(),
				cart.getPaymentInfo().getCardNum(), cart.getTotalPrice(), 0.00, 0.00, cart.getTotalPrice(), 0.0, 0.0 };
		saveCartTypes = new int[] { Types.INTEGER, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,
	...
	private void saveCartItem(CartItem item) throws DatabaseException {
		...
		saveCartItemParams = new Object[] { cartItem.getCartid(), cartItem.getProductid(),
				Integer.parseInt(cartItem.getQuantity()), Double.parseDouble(cartItem.getTotalprice()), 0.00, 0.00 };
		saveCartItemTypes = new int[] { Types.INTEGER, Types.INTEGER, Types.INTEGER, Types.DOUBLE, Types.DOUBLE,
				Types.DOUBLE };
		dataAccessSS.insert();
	}
	...
	private List<CartItem> getSavedCartItems(Integer cartId) throws DatabaseException {
		...
		// dataAccessSS.atomicRead(this);//this one works when testing
		// shoppingcart
