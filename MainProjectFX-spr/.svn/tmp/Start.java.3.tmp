...
public class Start extends Application {
	...
	private static Menu adminMenu,loginMenu,custMenu;
	private static MenuBar menuBar;
	private static MenuItem logoutMenuItem,loginMenuItem,exitMenuItem,retrieveCartMenuItem,reviewOrderMenuItem;
	...
	private static void addLogoutMenu(){
		loginMenu.getItems().remove(exitMenuItem);//to keep order
		loginMenu.getItems().add(logoutMenuItem);
		loginMenu.getItems().add(exitMenuItem);
	}
	private static void removeLogoutMenu(){
		loginMenu.getItems().remove(logoutMenuItem);
	}
	private static void addLoginMenu(){
		loginMenu.getItems().remove(exitMenuItem);
		loginMenu.getItems().add(loginMenuItem);
		loginMenu.getItems().add(exitMenuItem);
	}
	private static void removeLoginMenu(){
		loginMenu.getItems().remove(loginMenuItem);
	}
	private static void addCustMenuItem(){
		custMenu.getItems().add(retrieveCartMenuItem);
		custMenu.getItems().add(reviewOrderMenuItem);
	}
	public static void removeCustMenuItem(){
		custMenu.getItems().remove(retrieveCartMenuItem);
		custMenu.getItems().remove(reviewOrderMenuItem);
	}
	
	public static void setAfterLoginItems(){
		addCustMenuItem();//set customer options
		removeLoginMenu();
		addLogoutMenu();
	}
	public static void setAfterLogoutItems(){
		removeCustMenuItem();//set customer options
		addLoginMenu();
		removeLogoutMenu();
	}
	...
	private MenuBar createMenuBar() {
		...
		loginMenu = new Menu("File");
		loginMenuItem =login();
		exitMenuItem = exitApp();
		loginMenu.getItems().addAll(loginMenuItem,exitMenuItem);
		logoutMenuItem = logout();
		custMenu = new Menu("Customer");
		retrieveCartMenuItem=retrieveCart();
		reviewOrderMenuItem = reviewOrders();
		custMenu.getItems().addAll(onlinePurchase());
		adminMenu = new Menu("Administrator");
		adminMenu.getItems().addAll(maintainCatalogs(), maintainProducts());
		
		//add menus to menubar
		menuBar.getMenus().addAll(loginMenu, custMenu);
		return menuBar;
