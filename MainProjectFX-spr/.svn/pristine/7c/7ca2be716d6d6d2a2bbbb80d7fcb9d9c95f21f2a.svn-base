...
class DbClassOrder implements DbClass {
    ...
    private String submitOrderQuery = "INSERT into Ord "+
        ...
        "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"; 
    private String submitOrderItemQuery = "INSERT INTO orderitem (orderid, productid, quantity, " + ""
    		+ "totalprice, shipmentcost, taxamount) VALUES (?,?,?,?,?,?)";
  ...
  //implemented by segun
    void submitOrder(CustomerProfile custProfile, Order order) throws DatabaseException {
    	...
    	for (OrderItem oItem : this.order.getOrderItems()){
    		oItem.setOrderId(orderId);
    		submitOrderItem(oItem);
    	}
    	dataAccessSS.commit();
	...
	private void submitOrderItem(OrderItem item) throws DatabaseException {
        ...
        submitOrderItemParams = new Object[]{item.getOrderId(), item.getProductId(), item.getQuantity(),
        		item.getTotalPrice(), 100.0, item.getTaxAmount()};
    	...
    	OrderItemImpl orderItem = new OrderItemImpl();
    	 try {
             while(rs.next()){
            	 ...
            	 orderItem.setUnitPrice(totalPrice/qty);
    	...
    	orderData = new OrderImpl();
    	 try {
             while(rs.next()){
            	 ...
            	 orderData.setOrderId(rs.getInt("orderid"));
            	 DateTimeFormatter dtf = DateTimeFormatter.ofPattern("M/dd/yyyy");
            	 orderData.setDate(LocalDate.parse(rs.getString("orderdate"), dtf));
            	 orderData.setTotalPrice(rs.getDouble("totalpriceamount"));
             }
         }
         catch(SQLException e){
             throw new DatabaseException(e);
